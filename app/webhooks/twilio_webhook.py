...
task_executor = TaskExecutor(mcp_client) # TaskExecutor ahora también necesitará acceso a la email_tool, o la instanciará internamente
...
async def process_command_background(sender_number: str, message_content: str, is_audio: bool = False):
    ...
    try:
        ...
        # Extraer main_command y main_parameters
        ...
        if main_command != "unknown":
            logger.info(f"Interpreted data: {interpreted_data}")
            # Pasar el diccionario completo interpreted_data al task_executor
            await task_executor.execute_task_and_respond(interpreted_data, sender_number)
        else:
            logger.warning(f"Command interpretation failed or returned main_command 'unknown' for content: 	'{message_content[:100]}...	'")
